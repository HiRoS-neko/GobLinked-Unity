...
{
    [Tooltip("Range at which the enemy will detect goblins, in metres. Can be set randomly in right-click menu.")]public float visibleRange; //The range the enemy can see you out to
    ...
    [Tooltip("Whether or not to be actively searching for the player")]public bool search = true; //Whether or not the enemy is currently looking for a player
    ...
    [Tooltip("Rigidbody for the enemy, should be the one on this prefab.")]public Rigidbody2D body;    //The enemy rigidBody

    [Range(0, 100),Tooltip("The speed at which the enemy tracks the player. Can be randomized in right-click menu.")]public float speed;    //Speed to chase the goblin

    [Range(0,30),Tooltip("The enemies health value. 0-30")] public int health;    //Enemies maximum hp

    private int currHealth;    //Enemies current hp value
    
    [Range(0,100), Tooltip("The enemies default armour value.")]public int armor;    //Enemy armor value

    [Range(0,5), Tooltip("The damage value of the enemy")]public int attack;    //Enemy default attack value

    [Range(1,10), SerializeField, Tooltip("The speed multiplier for enemies. Since the player controller script is MEGA AUTISM")] private int enemySpeedMultiplier;
    ...
    void FixedUpdate()
    {
        ...
        {
            if (search && 
                (aiType == enemyTypes.Eyeball || aiType == enemyTypes.Rat || aiType == enemyTypes.Slime ||     //Behaviour for beelining to the goblins
                 aiType == enemyTypes.Farmer || aiType == enemyTypes.FarmerWoman))                             //Behaviour for beelining to the goblins
            {
                if (hitObjects[i].tag == "Goblin")
                {
                    body.velocity = (((Vector2) hitObjects[i].transform.position                        //Moves the enemy towards the goalbeen
                                      ...
                                      * PlayerController.SpeedMultiplier/enemySpeedMultiplier;          //Moves the enemy towards the goalbeen
